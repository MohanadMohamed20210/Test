...
int main() {
    ...
    std::cout << "Set the initial frequency => ";
    std::cin >> freq;  // Prompt user to enter initial PWM frequency

    signal(SIGINT, signal_handle);  // Set up signal handler for SIGINT (Ctrl+C)
    pwm = new PWM(freq);  // Create PWM object with initial frequency

    std::thread App(&PWM::startPWM, pwm);  // Start PWM thread

    // Main loop to dynamically update PWM frequency
    while (true) {
        std::cout << "Enter new frequency (0 to exit): ";
        std::cin >> freq;  // Prompt user to enter new PWM frequency
        if (freq == 0)
        {
            signal_handle(2);
